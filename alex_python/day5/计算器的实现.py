...
def handle_special_occactions(plus_andminus_operators,multiply_and_dividend):
    ...
    for index, i in enumerate(multiply_and_dividend):
        ...
        if i.endswith("*")or i.endswith("/"):
            multiply_and_dividend[index]+plus_andminus_operators[index]+multiply_and_dividend[index+1]
            del multiply_and_dividend[index+1]
            del plus_andminus_operators[index]
    return plus_andminus_operators,multiply_and_dividend
def compute(formula):
    '''这里计算的是不带括号的公式'''
    formula = formula.strip("()")# 去掉外面包的括号
    formula = remove_duplicates(formula)#去掉外重复的加减号
    plus_and_minus_operators = re.findall("[+-]",formula)
    multiply_and_dividend = re.split("[+-]",formula) #取出乘除公式
    if len(multiply_and_dividend[0].strip()) ==0:#这代表肯定是个减号
    ...
    return total_res
def calc(formula):
    '''计算机程序主入口，主要逻辑是先计算括号里的值，算出来以后再算乘除，再算加减'''
    parenthesise_flag = True
    calc_res = None #初始化运算结果为None，还没开始运算呢，当然为None
    while parenthesise_flag:
        m = re.search("\([^()]*\)",formula) #找到最里面的括号
        if m:
            #print('先算括号里面的值：'，m.group())
            ...
            parenthesise_flag = False #代表公式里面的括号已经全部被剥除了
